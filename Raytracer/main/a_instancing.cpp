...
void addTree(Group* g, int trunkTessel, float trunkHeight, float trunkRadius,
	...
	for (int i = 0; i < trunkTessel; ++i) {
		...
		Point end(trunkRadius * sin(angleNext), 0.0f,
				trunkRadius * cos(angleNext));
		g->add(
				new Quad(start, end - start, Vector(0.0f, trunkHeight, 0.0f),
						nullptr, nullptr));
	}
	...
	for (int i = 0; i < crownSteps; ++i) {
		...
		if (stepRadius > 0.0f) {
			...
			for (int j = 0; j < crownTessel; ++j) {
				...
				g->add(
						new Sphere(
								Point(stepRadius * sin(angle), height,
										stepRadius * cos(angle)), sphereRadius,
								nullptr, nullptr));
			}
		}
		g->add(
				new Sphere(Point(0.0f, trunkHeight, 0.0f), trunkRadius, nullptr,
						nullptr));
		g->add(
				new Sphere(
						Point(0.0f, trunkHeight + crownHeight - crownStep * 1.5,
								0.0f), trunkRadius, nullptr, nullptr));
	}
...
void a_instancing() {
	...
	for (int i = 0; i < 11; ++i) {
		...
		itree->translate(
				Vector(sin(angle) * circleRadius, 0.0f,
	...
	scene->add(
			new InfinitePlane(Point(0, 0, 0), Vector(0, 1, 0), nullptr,
					nullptr));
	scene->rebuildIndex();
