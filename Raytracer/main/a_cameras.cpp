...
RGBColor a2computeColor(const Ray& r) {
	...
	if (absfractional(theta / (2 * pi) * 90) < 0.03)
		color = RGBColor(0.9f, 0.5f, 0.5f) * 0.7f;
	if (absfractional(phi / (2 * pi) * 90) < 0.03)
		color = RGBColor(0.9f, 0.9f, 0.5f) * 0.7f;
	return color;
}
...
void a_cameras() {
	...
	FisheyeCamera fcam(Point(0, 0, 0), Vector(1, 0, 0), Vector(1, 0, pi / 2), pi, pi, 1);
	Renderer rBonus(&fcam, 0);
	rBonus.test_render2(img);
	img.writePNG("aFish.png");

	EnvironmentCamera ecam(Point(0, 0, 0), Vector(1, 0, pi / 2), Vector(1, 0, 0), pi, pi, 1);
	Renderer rBonus2(&ecam, 0);
	rBonus2.test_render2(img);
