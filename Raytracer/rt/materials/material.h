...
public:
	 ...
	 */
	virtual RGBColor getReflectance(const Point& texPoint, const Vector& normal, const Vector& outDir,
			const Vector& inDir) const = 0;
	...
	struct SampleReflectance {
		...
		SampleReflectance() :
				direction(0, 0, 1), reflectance(0.0f, 0.0f, 0.0f) {
		}
	};

	enum Sampling {
		SAMPLING_NOT_NEEDED, SAMPLING_SECONDARY, SAMPLING_ALL
	};
