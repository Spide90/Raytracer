...
RGBColor RayMarchingIntegrator::getRadiance(const Ray& ray) const {
	...
	if (intersection) {
		...
		if (intersection.solid->material == nullptr) {
			...
			emission = intersection.solid->material->getEmission(local,
