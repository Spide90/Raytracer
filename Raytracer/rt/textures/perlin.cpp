...
float smooth_noise(int x, int y, int z){
	...
	float adjacent = (noise(x,y,z-1) + noise(x,y,z+1) + noise(x,y-1,z) + noise(x,y+1,z) + noise(x-1,y,z) + noise(x+1,y,z))/16;
...
//		interpol = interpol + fabs(lerp3d(smooth_noise(int_x, int_y, int_z), smooth_noise(int_x + 1, int_y, int_z),
